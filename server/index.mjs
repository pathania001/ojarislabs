...
const server = createServer(async (req, res) => {
  try {
    ...
    // API routes must be registered before static-file handling.
    if (url.pathname === "/api/health" || url.pathname === "/api/health/") {
      if (req.method !== "GET" && req.method !== "HEAD") {
        return json(res, 405, { ok: false, error: "method_not_allowed" });
      }
      return json(res, 200, { ok: true, service: "ojarislabs-contact-api" });
    }
    ...
    if (!res.headersSent) {
      json(res, 500, { ok: false, error: "server_error", message: "Something went wrong." });
    }
  }
});
