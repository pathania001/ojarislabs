...
- **Run the site:** `npm run dev` (starts `serve` on `http://localhost:3000`). This is a static site — there is no backend, database, or build step. `npm run build` is intentionally a no-op.
- **Clean URLs are server-provided, not files.** Pages are flat files (`services.html`, `about.html`, …) but are served at extensionless paths (`/services`, `/about`) because `serve.json` sets `cleanUrls: true`. If you serve the folder with a different static server (e.g. `python -m http.server`), those clean paths will 404 — either use `npm run dev` or link with `.html`. In-page links intentionally use the clean form (`/services`).
- **Asset paths are root-absolute** (`/css/...`, `/js/...`, `/assets/...`), so the site must be served from the repository root, not a subfolder.
