...
- **Validate before shipping:** `npm run lint` (stylelint + htmlhint) and `npm run check:links` (broken links, missing `#fragments`, duplicate titles/descriptions, missing/multiple H1, missing canonical, missing image alt). Always rebuild first: `npm run build:site && npm run lint && npm run check:links`.
